...
function Player() {
  ...
  useEffect(() => {
    ...
    setUserInfo(user);
    fetch(`http://localhost:8001/movies/${params.id}`)
      .then((response) => {
  ...
  // play function
  function play() {
    fetch("http://localhost:8001/user/play", {
      method: "POST",
      headers: {
  ...
  function closeplayer() {
    fetch(`http://localhost:8001/user/closePlayer/${userMovieModel._id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        authorization: `Bearer ${userInfo.token}`,
            ...
            <div className="container player_container">
              ...
              <div className="video_container">
                <video controls autoPlay ref={videoPlayer}>
                  <source
                    src={`http://localhost:8001/movies/stream/${singleMovie?._id}`}
                  />
                </video>
              </div>
            </div>
